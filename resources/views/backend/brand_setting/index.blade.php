...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <h4 class="header-title">@lang('brand_setting.view_brand')</h4>
                        ...
                        <div class="tab-content">
                                ...
                                <table class="table table-striped dt-responsive nowrap w-100" id="myTable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>@lang('brand_setting.brand_name')</th>
                                            <th>@lang('brand_setting.order_by')</th>
                            ...
                            <div class="tab-pane" id="users-tab-deleted">
                                ...
                                $trashed=  brand_setting::onlyTrashed()->get();
                                $sl=1;
                                @endphp
                                ...
                                <table id="alternative-page-datatable" class="table table-striped dt-responsive nowrap w-100">
                                    ...
                                    <tbody class="table-border-bottom-0">
                                        ...
                                        <tr>
                                            <td>{{$sl++}}</td>
                                            <td>
                                                @if($lang == 'en') {{$v->brand_name_en}} @elseif($lang == 'bn') {{$v->brand_name_bn}} @endif
                                            </td>
                                            <td>
                                                {{$v->order_by}}
                                            </td>
                                            <td>
                                                <a href="{{ url('brand_restore') }}/{{ $v->id }}" class="btn btn-sm btn-info">@lang('common.restore')</a>
                                                <a href="{{ url('brand_delete') }}/{{ $v->id }}" class="btn btn-sm btn-danger">@lang('common.permenantly_delete')</a>  
                                            </td> 
                                        </tr>
                                        @endforeach
                                        @endif
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var table = $('#myTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('brand_setting.index') }}",
                columns: [
                    {data: 'sl', name: 'sl'},
  ...
  <script>
        function brandStatusChange(id)
        {
            ...
            if(id > 0)
            {
                var message = @json( __('brand_setting.status_message') );
